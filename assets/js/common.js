(function ($) {
	...
	function videosLoaded(el, callback) {
		...
		for (var i = 0; i < el.length; i++) {
			...
			if (el[i].readyState > 3) {
				...
				//set listener if not
				$(el[i]).on('canplaythrough', function () {
					done();
				});
			}
		}
	...
	function onePage() {
		...
		var skills = {
			...
			numbers: function (bar, i) {
				...
				var countEl = this.numbersEl[i],
					countVal = parseInt(countEl.getAttribute('data-counters-number')),
		...
		$(tooltipForElems)
			.hover(function () {
					...
					newTooltip.style.top = posY - parseInt(getComputedStyle(newTooltip).height) / 2 + 'px';
					newTooltip.style.left = posX - parseInt(getComputedStyle(newTooltip).width) / 2 + 'px';
	...
	var scrolling = {
		...
		init: function () {
			...
			//this.scrollToHash();
			if (!$.browser.mobile || !$.platform.mac) this.custom();
		},
	...
	var animateOnScroll = {
		...
		setHandlers: function () {
			...
			$(this.animateItems).on($.support.animationEnd + ' ' + $.support.transitionEnd, function () {
		...
		destroy: function () {
			...
			$(this.animateItems).off($.support.animationEnd);
	...
	var parallax = {
				...
				if (visible) {
					...
					if (this.options[i].fading === 'true') {
						...
						if ($.support.cssFilter) {
							el.style[$.support.cssFilter] = 'brightness(' + fading + '%)';
						} else {
							//ugly fallback
					...
					$.support.cssTransform3d ? el.style[$.support.cssTransform3d] = 'translate3d(0,' + -translateY + 'px,0) scale3d(' + scale + ', ' + scale + ', 1)' : el.style[$.support.cssTransform2d] = 'translate(0,' + -translateY + 'px) scale(' + scale + ')';
				}
			}
		},
		...
		stopParallax: function (el) {
			...
			el.style.removeProperty($.support.cssTransform3d ? $.support.cssTransform3d : $.support.cssTransform2d);
			//if ($.support.cssFilter) el.style.removeProperty($.support.cssFilter);
		}
	}
	parallax.init();
	...
	*/
	...
	function sliderModal($elems) {
		...
		function showItem(e, $this) {
			...
			var $this = $this || e.target,
				url = $($this).attr('href'),
				i = parseInt($($this).attr('data-sliderModal-item'));
			...
			//video case
				...
				if (/.*youtube\.com.*/i.test(url) || /.*vimeo\.com.*/i.test(url)) {
					showModal('<iframe class="modal__video" src="' + url + '" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>', 'video');
				} else {
		...
		function setArrows(i) {
			...
			if (i === 0) {
				...
				sliderModalNextArrow.one('click', function (e) {
					e.stopPropagation();
					sliderModalPrevArrow.off('click');
				...
				sliderModalPrevArrow.one('click', function (e) {
					e.stopPropagation();
					sliderModalNextArrow.off('click');
		...
		var methods = {
			...
			goToSlide: function (i) {
				...
				if (this.prevItem !== null) {
					...
					var slideTransDur = getComputedStyle($items[this.prevItem])[$.support.transition + 'Duration'],
						slideTransDel = getComputedStyle($items[this.prevItem])[$.support.transition + 'Delay'];

					slideTransDur = (slideTransDur).toLowerCase().indexOf('ms') > -1 ? parseInt(slideTransDur) : parseFloat(slideTransDur) * 1000;
					slideTransDel = (slideTransDel).toLowerCase().indexOf('ms') > -1 ? parseInt(slideTransDel) : parseFloat(slideTransDel) * 1000;
	...
	function customizer() {
		...
		//helper
		function getRutThemeRules() {
			var rules = document.styleSheets[6].rules,
				result = '';
	 ...
	 * Font Lato Thin fix
	 */
	if ($.platform.mac && $.browser.safari || !$.platform.mac && $.browser.webkit) {
		var allElems = document.getElementsByTagName('*');
		for (var i = 0; i < allElems.length; i++) {
			if (getComputedStyle(allElems[i]).fontWeight <= 300) {
				allElems[i].style.webkitTextStroke = '0.2px';
			}
		}
	}
